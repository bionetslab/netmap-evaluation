...
if __name__=='__main__':
    ...
    model_name = 'blood-10x-rep1-simpleaf-norm'
    model_output_dir = op.join(output_dir, model_name)
    os.makedirs(model_output_dir, exist_ok=True)
    adata = sc.read_h5ad('/data_nfs/og86asub/netmap/netmap-evaluation/netmap/data/blood/reprocessed/blood-10x-rep1-simpleaf.h5ad')
    train_model(adata, output_dir=model_output_dir, model_name = model_name, scale='norm')

    model_name = 'blood-10x-rep1-simpleaf-X'
    model_output_dir = op.join(output_dir, model_name)
    os.makedirs(model_output_dir, exist_ok=True)
    adata = sc.read_h5ad('/data_nfs/og86asub/netmap/netmap-evaluation/netmap/data/blood/reprocessed/blood-10x-rep1-simpleaf.h5ad')
    train_model(adata, output_dir=model_output_dir, model_name = model_name, scale='X')

    model_name = 'blood-10x-rep1-simpleaf-count'
    model_output_dir = op.join(output_dir, model_name)
    os.makedirs(model_output_dir, exist_ok=True)
    adata = sc.read_h5ad('/data_nfs/og86asub/netmap/netmap-evaluation/netmap/data/blood/reprocessed/blood-10x-rep1-simpleaf.h5ad')

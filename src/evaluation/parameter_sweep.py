...
if __name__== '__main__':
    ...
    outdir = f'/data_nfs/og86asub/netmap/netmap-evaluation/results/summaries_sweep/{net}'
    ...
    background_type = ['zeros', 'randomize', 'data']
    ...
    for hd in hyper.keys():   
        for dp in dropout_percentage:
            for mt in model_type:
                ...
                for xai in ['GuidedBackprop', 'GradientShap', 'Deconvolution']:
                    
                    if xai == 'GradientShap':
                        bg_modes = ['zeros', 'randomize', 'data']
                    else:
                    ...
                    for bg in bg_modes:
                        ...
                        overlaps_ungrouped.to_csv(op.join(outdir, f'{xai}_{hd}_{dp}_{mt}_{bg}_overlaps_global_top_k.tsv'), sep='\t')

                        
                        area_results = area_results.reset_index()
                        area_results.to_csv(op.join(outdir, f'{xai}_{hd}_{dp}_{mt}_{bg}_area_over_diagnoal.tsv'), sep='\t')
                        ...
                        write_config(c = collect_results, file= op.join(outdir, f'{xai}_{hd}_{dp}_{mt}_{bg}_clustering_score.json'))

...
from src.methods.netmap.netmap_config import NetmapConfig
...
if __name__ == "__main__":
    # 1. Define the default path (your original hardcoded one)
    default_config = op.join(op.dirname(__file__), '../../configurations/pipelines/pipeline_config_2.yaml')

    # 2. Set up the argument parser
    parser = argparse.ArgumentParser(description="Run the data pipeline.")
    
    # 3. Add the argument
    # Using --config allows you to pass it as an optional flag
    parser.add_argument(
        "--config", 
        type=str, 

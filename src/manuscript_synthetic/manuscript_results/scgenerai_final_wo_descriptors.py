...
if __name__ == "__main__":
    ...
    model.fit(ex_data_us, nepochs = 100, model_depth =2, early_stopping=True, device_name = 'cuda')
    model.predict_networks(ex_data_us, PATH = '.')



    res_files = os.listdir('./results_wo_descriptors')
    chunk_size = 300
    ...
    for i in range(0, len(res_files), chunk_size):
        ...
        batch = res_files[i:j]
        batch_df = pd.concat([pd.read_csv(op.join('./results_wo_descriptors', res_file)).assign(cell_name=res_file.split('_')[-1].replace('.csv', '')) for res_file in batch], ignore_index=True)
    ...
    pivot_table.to_csv('./scgenerai_pandas_wo_descriptors.tsv', sep ='\t')


    ada2 = sc.AnnData(X = aa, obs = ex_data_descriptors.iloc[0:aa.shape[0]])
    ada2.write_h5ad('./scgenerai_wo_descriptors.h5ad')
    sc.pp.scale(ada2)
    ...
    sc.pl.umap(ada2, save='_with_descriptors_trained_wo.pdf', color =['cell_type_epi', 'patient_id'])

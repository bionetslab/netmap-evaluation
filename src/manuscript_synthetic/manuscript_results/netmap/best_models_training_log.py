...
if __name__== '__main__':
    ...
    parser.add_argument(
    "--data_path",  # name on the CLI - drop the `--` for positional/required parameters
    type=str,
    default=None,  # default if nothing is provided
    )
    ...
    os.makedirs(sumout, exist_ok=True)
    ...
    print(adata.shape)
    ## Get the data matrix from the CustomAnndata obeject
                ...
                for xai in ['GradientShap']:
                    ...
                    for bg in bg_modes:
                        ...
                        for raw in attribution_raw:
                            ...
                            overlaps_ungrouped.to_csv(op.join(sumout, f'{xai}_{hd}_{dp}_{mt}_{bg}_{raw}_overlaps_global_top_k.tsv'), sep='\t')

                                                                                    
                            aggregated_performance = compute_metrics.compute_aggregated_grn_result(grn_adata2, nets, cluster_col = 'spectral_remap' )
                            aggregated_performance.to_csv(op.join(sumout, f'{xai}_{hd}_{dp}_{mt}_{bg}_{raw}_aggregated_performance.tsv'), sep = '\t')
                            
                            aggregated_performance_scaled = compute_metrics.compute_aggregated_grn_result(grn_adata2, nets, cluster_col = 'grn' )
                            aggregated_performance_scaled.to_csv(op.join(sumout, f'{xai}_{hd}_{dp}_{mt}_{bg}_{raw}_aggregated_performance_grn.tsv'), sep = '\t')
                            
                            print(sumout)
                            
                            area_results = area_results.reset_index()
                            area_results.to_csv(op.join(sumout, f'{xai}_{hd}_{dp}_{mt}_{bg}_{raw}_area_over_diagnoal.tsv'), sep='\t')
                            ...
                            write_config(c = collect_results, file= op.join(sumout, f'{xai}_{hd}_{dp}_{mt}_{bg}_{raw}_clustering_score.json'))
